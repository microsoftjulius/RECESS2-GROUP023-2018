...
#### Our code code for the user interface --------------------------------------------------------------
  output$ui <- renderUI({
    if (user_input$authenticated == FALSE) {
      ##### User Interface code for the login page
      fluidPage(
        fluidRow(
      ...
      #### The shiny app's code for the user interface
      fluidPage(
        fluidRow(
          # Slider input
...
#### PASSWORD server code to check all constraints ---------------------------------------------------- 
  ...
  observeEvent(input$login_button, {
    credentials <- read.csv("credentials.csv", header = T)
    ...
    if (input$login_button == num_fails_to_lockout & 
      ...
      if (length(row_username) == 1) {
        ...
        save(credentials, "credentials.csv")
      }
    }
